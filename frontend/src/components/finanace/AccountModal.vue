        ...
        >
          <DialogPanel class="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 px-6 py-5">
              <DialogTitle class="text-base font-semibold text-slate-900">เพิ่มบัญชี</DialogTitle>
              <button
                @click="$emit('close')"
...
import type { CreateAccountPayload } from '@/types/finance'
...
const local = reactive<CreateAccountPayload>({
  accountName: '',
  accountCode: '',
  balance: 0,
})
...
watch(
  ...
  (opened) => {
    if (!opened) return
    Object.assign(local, { accountName: '', accountCode: '', balance: 0 })
  },
)

function handleSubmit() {
  emit('submit', { ...local } satisfies CreateAccountPayload)
}
</script>
