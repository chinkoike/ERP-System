<template>
  <aside
    ...
    <!-- Logo -->
    <div class="flex h-16 items-center border-b border-slate-100 px-4" :class="collapsed ? 'justify-center' : 'gap-3'">
      ...
      <span v-if="!collapsed" class="text-sm font-semibold text-slate-900 tracking-tight">ERP System</span>
    </div>
    ...
    <div class="border-t border-slate-100 p-2 space-y-0.5">
      ...
      <div
        ...
        <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-slate-200 text-[11px] font-semibold text-slate-600">
          {{ userInitial }}
        </div>
        <div v-if="!collapsed" class="min-w-0 flex-1">
          <div class="truncate text-sm font-medium text-slate-900">{{ authStore.user?.username }}</div>
      ...
      <button
        ...
        <ChevronIcon :class="collapsed ? 'rotate-180' : ''" class="h-4 w-4 flex-shrink-0 transition-transform duration-200"/>
...
const DashboardIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M4 5a1 1 0 011-1h4a1 1 0 011 1v5a1 1 0 01-1 1H5a1 1 0 01-1-1V5zm10 0a1 1 0 011-1h4a1 1 0 011 1v2a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zm10-3a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1h-4a1 1 0 01-1-1v-7z' }),
])}
const InventoryIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10' }),
])}
const SalesIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z' }),
])}
const PurchasingIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const FinanceIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const ReportIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const UsersIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const LogoutIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const ChevronIcon = { render: () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M11 19l-7-7 7-7m8 14l-7-7 7-7' }),
])}

const navItems = [
  { name: 'dashboard',  label: 'Dashboard',  to: '/dashboard',  icon: DashboardIcon  },
  { name: 'inventory',  label: 'Inventory',  to: '/inventory',  icon: InventoryIcon  },
  { name: 'sales',      label: 'Sales',      to: '/sales',      icon: SalesIcon      },
  { name: 'purchasing', label: 'Purchasing', to: '/purchasing', icon: PurchasingIcon },
  { name: 'finance',    label: 'Finance',    to: '/finance',    icon: FinanceIcon    },
  { name: 'report',     label: 'Report',     to: '/report',     icon: ReportIcon     },
  { name: 'users',      label: 'Users',      to: '/users',      icon: UsersIcon      },
]
</script>
